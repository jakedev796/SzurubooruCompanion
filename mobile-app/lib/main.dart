  ...
  Future<void> _checkOverlayPermissionOnResume() async {
    ...
    try {
      ...
      if (hasOverlay && settingsWantBubble && !_showFloatingBubble) {
        ...
        if (mounted) await _saveSettingsInternal(validate: false, checkPermissions: false);
      } else if (hasOverlay && settingsWantBubble) {
        // Permission was granted while away (e.g. user granted in Settings and returned).
        // UI state may already have bubble on; ensure the overlay is actually started.
        if (mounted) await _applyBubbleAndNotificationFromState();
      }
    } catch (e) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) => DraggableScrollableSheet(
        ...
        builder: (context, scrollController) => _JobDetailSheetContent(
          ...
          booruUrl: booruUrl,
          onOpenPost: (booru, postId) => _openPostLink(booru ?? '', postId),
        ),
      ),
    );
  }
    ...
    if (shouldRequest == true && mounted) {
      try {
        ...
        if (granted && settings.showFloatingBubble && mounted) {
          ...
          final folders = await settings.getScheduledFolders();
          final folderSyncEnabled = folders.any((f) => f.enabled) && settings.showPersistentNotification;
          if (folderSyncEnabled) {
            await startCompanionForegroundService(
              folderSyncEnabled: true,
              bubbleEnabled: true,
...
class _JobDetailSheetContent extends StatefulWidget {
  ...
  @override
  State<_JobDetailSheetContent> createState() => _JobDetailSheetContentState();
}

/// Sort tags: digit-prefixed first (123), then A-Z (case-insensitive). Matches frontend.
...
class _JobDetailSheetContentState extends State<_JobDetailSheetContent> {
  Job? _job;
  bool _loading = true;
  ...
  Widget build(BuildContext context) {
      ...
      actionButtons.add(
        Padding(
          ...
          child: ElevatedButton(
            ...
            child: Text(
              _actionLoading == label.toLowerCase() ? '${label}ing...' : label,
            ),
          ),
        ),
      );
    ...
    return ListView(
      ...
      children: [
        ...
        _detailRow('Filename', Text(job.originalFilename ?? '-', style: const TextStyle(fontSize: 13))),
        _detailRow('Safety', Text(job.safetyDisplay, style: const TextStyle(fontSize: 13))),
        _detailRow('Upload User', Text(job.szuruUser ?? '-', style: const TextStyle(fontSize: 13))),
        if (job.szuruPostId != null && widget.booruUrl != null)
          _detailRow(
            'Szuru Post ID',
            InkWell(
              onTap: () => widget.onOpenPost(widget.booruUrl, job.szuruPostId!),
              child: Text(
                'View post #${job.szuruPostId}',
                style: TextStyle(
                  fontSize: 13,
          ...
          _detailRow(
            ...
            Wrap(
              ...
              children: widget.booruUrl != null
                  ? job.relatedPostIds!
                      .map((id) => InkWell(
                            onTap: () => widget.onOpenPost(widget.booruUrl, id),
                            child: Text(
                              '#$id',
                              style: TextStyle(
                                fontSize: 13,
                      ...
                      .toList()
                  : [Text(job.relatedPostIds!.map((id) => '#$id').join(', '), style: const TextStyle(fontSize: 13))],
            ),
          ),
        _detailRow('Skip Tagging', Text(job.skipTagging ? 'Yes' : 'No', style: const TextStyle(fontSize: 13))),
        if (sortedTagsFromSource.isNotEmpty)
          _detailRow(
            'Tags from source',
            Wrap(
              children: sortedTagsFromSource.map((t) => _tagChip(t, 'source')).toList(),
            ),
          ),
        if (sortedTagsFromAi.isNotEmpty)
          _detailRow(
            'Tags from AI',
            Wrap(
              children: sortedTagsFromAi.map((t) => _tagChip(t, 'ai')).toList(),
            ),
          ),
            ...
            Column(
              ...
              children: [
                Padding(
                  ...
                  child: Text(
                    'Combined from source, AI, and metadata. Colors match origin above.',
                ...
                Wrap(
                  children: sortedTagsOnSzuru.map((t) {
                    ...
                    final variant = sourceLower.contains(key) ? 'source' : (aiLower.contains(key) ? 'ai' : 'other');
        ...
        _detailRow('Retries', Text('${job.retryCount}', style: const TextStyle(fontSize: 13))),
        _detailRow('Created', Text(_formatDate(job.createdAt), style: const TextStyle(fontSize: 13))),
        _detailRow('Updated', Text(_formatDate(job.updatedAt), style: const TextStyle(fontSize: 13))),
      ],
    );
  }
