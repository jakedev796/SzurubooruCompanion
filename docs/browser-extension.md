...
### Media the backend cannot reach

Normally the extension sends a URL and the backend downloads it. That cannot work when the URL only means something inside your browser:

- pages served from `localhost`, `127.0.0.1`, or `[::1]` — the backend would resolve those to itself
- `blob:` and `data:` media generated by the page

In those cases the extension reads the bytes itself and uploads them to CCC directly, so the post is created normally. The page URL is recorded as the post's source. A few limits apply:

- media that can only be read from inside the page (`blob:`) is capped at 32 MB
- `file:///` URLs cannot be read; upload those from the dashboard or mobile app
- because there is no page for gallery-dl to parse, tags and metadata come from AI tagging alone

LAN addresses (for example `192.168.x.x` or `nas.local`) are still sent as URLs, since the backend may well be able to reach them.
